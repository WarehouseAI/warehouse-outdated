...
func Login(userInfo *Request, userProvider UserProvider, sessionCreator SessionCreator, logger *logrus.Logger, ctx context.Context) (*Response, *r.Session, *httputils.ErrorResponse) {
	...
	if err := bcrypt.CompareHashAndPassword([]byte(user.Password), []byte(userInfo.Password)); err != nil {
		...
		return nil, nil, httputils.NewErrorResponse(httputils.BadRequest, "Invalid credentials")
	}
	...
	if sessErr != nil {
		...
		return nil, nil, httputils.NewErrorResponseFromDBError(sessErr.ErrorType, sessErr.Message)
	}

	return &Response{UserId: user.Id}, session, nil
}
